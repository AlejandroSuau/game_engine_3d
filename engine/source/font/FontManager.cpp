...
std::shared_ptr<Font> FontManager::GetFont(const std::string& path, int size) {
    ...
    FT_Error result = FT_New_Memory_Face(
        m_fontLibrary, reinterpret_cast<FT_Byte*>(buffer.data()), buffer.size(), 0, &face
    );
    if (result != FT_Err_Ok) {
        return nullptr;
    }
    ...
    int maxDimension = static_cast<int>(std::sqrt(128.f) * (lineHeight + 1));
    int textureWidth = 1;
    ...
    const std::size_t stride = textureWidth * 4;
    const std::size_t totalBytes = static_cast<std::size_t>(textureWidth * textureHeight * 4);
    ...
    for (int c = 0; c < 128; ++c) {
        ...
        for (uint32_t row = 0; row < bmp.rows; ++row) {
            for (uint32_t col = 0; col < bmp.width; ++col) {
                ...
                const std::size_t idx = static_cast<std::size_t>(y) * stride + x * 4;
