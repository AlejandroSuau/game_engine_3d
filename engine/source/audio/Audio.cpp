...
bool Audio::IsPlaying() const {
    return (m_sound) ? ma_sound_is_playing(m_sound.get()) : false;
}

void Audio::SetVolume(float volume) {
    if (!m_sound) { return; }

    ma_sound_set_volume(m_sound.get(), std::clamp(volume, 0.f, 1.f));
}
...
float Audio::GetVolume() const {
    return (m_sound) ? ma_sound_get_volume(m_sound.get()) : 0.f;
}
...
std::shared_ptr<Audio> Audio::Load(const std::string& path) {
    ...
    auto result = ma_decoder_init_memory(
        audio->m_buffer.data(), audio->m_buffer.size(), nullptr, audio->m_decoder.get());
